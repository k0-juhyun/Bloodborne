...
namespace bloodborne
{
    ...
    public class BossBeta : MonoBehaviour
    {
        ...
        public enum BossAttackState                 // 공격 패턴 상태
        {
            SickelCombo1,
            SickelCombo2,
            SickelCombo3,
        ...
        public bool Attacking = false;                      // 공격 한번만 호출용
        ...
        void Update()
        {
            ...
            switch (attackState)
            {
                case BossAttackState.SickelCombo1:
                    UpdateSickelCombo1();
                    break;
                case BossAttackState.SickelCombo2:
                    UpdateSickelCombo2();
                    break;
                case BossAttackState.SickelCombo3:
                    UpdateSickelCombo3();
                    break;
                case BossAttackState.SwordCombo1:
                    UpdateSwordCombo1();
                    break;
                case BossAttackState.SwordCombo2:
                    UpdateSwordCombo2();
                    break;
                case BossAttackState.GunCombo:
                    UpdateGunCombo();
                    break;
            }
        }
        ...
        {
            // 공격 상태
            // 페이즈에 따라 다른 공격을 순서대로 하고 싶다
            // 만약 현재 페이즈가 1페이즈라면
            if (bossPhase == BossPhase.Phase1)
            {
                if (Attacking == false)
                {
                    // 낫 공격을 순서대로 호출하고 싶다 range [0, 2] = 3, 낫3 - 3 공격에서 0으로 초기화 해야함
                    for (int i = 0; i < 3; i++)
                    {
                        BossAttackState b1 = (BossAttackState)(i);
                        attackState = b1;
                        // 모든 공격이 끝날때, Attacking을 false 로 한다
                        Attacking = true;
                    }
                }
            ...
            // 만약 현재 페이즈가 2페이즈 또는 3페이즈 라면
            else //if (bossPhase == BossPhase.Phase2 || bossPhase == BossPhase.Phase3)
            {
                if (Attacking == false)
                {
                    // 칼 공격을 순서대로 호출하고 싶다, 총 공격은 칼공격 후 무조건 호출 or 50 퍼 확률로 호출
                    if (isSword1done == false)
                    {
                        attackState = BossAttackState.SwordCombo1;
                        
                    }
                    else
                    {
                        attackState = BossAttackState.SwordCombo2;
                    }
                }
            }
        }
        ...
        private void UpdateSwordCombo2()
        {
            ...
            switch (attackSubState)
            {
                ...
                case AttackSubState.Attack3:
                    ...
                    {
                        // 끝나면 총공격을 호출한다
                        attackState = BossAttackState.GunCombo;
                        isSword1done = false;
                    }
                    break;
            }
        ...
        private void UpdateIdle()           // 공격이 끝나면 idle 상태로 옴
        {
            ...
            {
                // 공격 상태로 변환
                bossState = BossPatternState.Attack;
        ...
        {
            // 낫공격3으로 상태 변화시킨다
            attackState = BossAttackState.SickelCombo3;
            print("SickelCombo3");
            FindMoveTargetPos();
            anim.SetBool("Leg", true);
        }
        ...
        {
            // 낫공격2으로 상태 변화시킨다
            attackState = BossAttackState.SickelCombo2;
            print("SickelCombo2");
            FindMoveTargetPos();
            anim.SetBool("Leg", true);
        }
        ...
        {
            // 낫공격1으로 상태 변화시킨다
            attackState = BossAttackState.SickelCombo1;
            print("SickelCombo1");
            FindMoveTargetPos();
            anim.SetBool("Leg", true);
        }
